...
import { PKState } from "./state.js";
import { PKREC } from "./recorder.js";
import { SaveLocal } from "./local.js";

export const PKAudioList = [];
...
function PKAE () {
	...
	q.init = function (el_id ) {
		...
		// init libraries
		q.ui     = new PKUI(q); // q._deps.ui ( q );
		new PKUI_FX(q); //q._deps.uifx ( q );
		q.engine = new PKEng(q); // new q._deps.engine ( q );
		q.state  = new PKState(4, q); // new q._deps.state ( 4, q );
		q.rec    = new PKREC(q); // new q._deps.rec ( q );
		q.fls    = new SaveLocal(q); // new q._deps.fls ( q );
		addFXPGEQ(this);
		addFXAuto(this);
...
export const PKAudioEditor = new PKAE ();
